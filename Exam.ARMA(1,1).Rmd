...
```{r}
# Estrazione dei parametri stimati dal risultato MCMC
alpha_hat <- outputmcmcARMA_INF$mean$alpha
beta_hat <- outputmcmcARMA_INF$mean$beta
tau_hat <- outputmcmcARMA_INF$meant$tau
m0 <- outputmcmcARMA_INF$sims.list$m0
params <- c(alpha_hat, beta_hat, tau_hat, m0)

# Verifico che i parametri stimati non siano NA
if (any(is.na(params))) {
  stop("Ci sono parametri stimati che sono NA.")
}

# Verifico che i dati non contengano NA o infiniti
if (any(is.na(infData)) || any(is.infinite(infData))) {
  stop("I dati contengono NA o valori infiniti.")
}
...
logLik_arma <- function(params, data) {
  ...
  for (i in 2:N) {
    ...
    # Calcolo la log-verosimiglianza e verifica eventuali problemi 
    logLik_i <- dnorm(data[i], mean=mu[i], sd=sqrt(1/tau), log=TRUE)
    
    #volendo questi li possiamo eliminare 
    if (is.na(logLik_i)) {
      print(paste("NA in logLik a i =", i))
      print(paste("data[i] =", data[i]))
      print(paste("mu[i] =", mu[i]))
...
# Calcola la log-verosimiglianza per i dati Inf
logLik_value <- logLik_arma(params, infData)

# Numero di parametri stimati (lo sappiamo già quanti sono)
k <- length(params)
